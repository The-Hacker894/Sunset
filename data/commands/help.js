...
module.exports.run = (client, message, args, data, game, announcement) => {
  ...
  .setColor(data.embedcolor)
    .setTitle('Commands')
    .addField('**Information**','`help` `temp` `ping` `info` `usage` `announcement` `uptime` `avatar` `profile` `whois`')
    ...
    .addField('**Moderation**','`rules` `lockdown` `channelsettings` `ban` `unban` `kick` `purge` `mkchannel` `channeldelete` `setnick`')
