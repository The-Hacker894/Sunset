...
module.exports.run = (client, message, args, data, game, announcement) => {
  ...
  .setColor(data.embedcolor)
    .setTitle('Commands')
    .addField('**Information**','`help` `temp` `ping` `info` `usage` `announcement` `uptime`')
    .addField('**Server Info**','`invite` `serverinfo` `avatar` `profile` `whois`')
    ...
    .addField('**Moderation**','`warn` `warnings` `rules` `lockdown` `channelsettings` `mute` `unmute` `ban` `unban` `kick` `purge` `mkchannel` `delchannel` `setnick`')
