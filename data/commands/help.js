...
module.exports.run = (client, message, args, data, game, announcement) => {
  ...
  .setColor(data.embedcolor)
    .setTitle('Commands')
    .addField('**Information**','`help` `ping` `info` `usage` `announcement` `uptime` `avatar` `profile` `whois`')
    ...
    .addField('**Content Generation**', '`url` `base64` `qrcode` `lastqr` `createtxt` `lasttxt`')
